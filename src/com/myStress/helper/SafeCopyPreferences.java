 ...
 * This function is used by {@link com.myStress.myStress_shortcut}, {@link com.myStress.myStress_record_tab} and {@link com.myStress.myStress_configure_remote} to handle shortcuts
 *
 */
public class SafeCopyPreferences 
{
	...
	static public void copyPreferences(Context context, File shortcutFile)
	{
        ...
        tables = settings.getBoolean("myStress_local::TablesExists", false);	
        tables2 = settings.getBoolean("myStress_local::Tables2Exists", false);
        first_start = settings.getBoolean("myStress_local::first_start", false);
        copy_template = settings.getBoolean("myStress_local::copy_template", false);
    	...
    	File preferenceFile = new File(context.getFilesDir(), "../shared_prefs/com.myStress_preferences.xml");
    	// if file does not exist use a path that is usually used by GalaxyS in 2.3!!!
    	if (preferenceFile.exists() == false)
    		preferenceFile = new File("/dbdata/databases/com.myStress/shared_prefs/com.myStress_preferences.xml");
        ...
        {
        	Log.e("myStress", "Could not copy template into default settings!");
        }
		...
		editor.putBoolean("myStress_local::TablesExists", tables);
		editor.putBoolean("myStress_local::Tables2Exists", tables2);
		editor.putBoolean("myStress_local::first_start", first_start);
		editor.putBoolean("myStress_local::copy_template", copy_template);
