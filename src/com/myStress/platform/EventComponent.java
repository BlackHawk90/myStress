...
import com.myStress.myStress_remote;
...
public class EventComponent implements Runnable 
{
	...
	/**
	 * Reference to {@link myStress_remote} service
	 */
	public  myStress_remote  myStress;
	/**
	 ...
	 * @param myStress Reference to the {@link myStress_remote} service which started this component
	 * @param IPAddress String with the IP address to connect to 
	 * @param IPPort Port to connect to at the given IP address
	 */
	public boolean startEC(myStress_remote myStress, String IPAddress, String IPPort)
	{
		current_TCPClient	= new TCPClient();
		// was connection successful?
		if (current_TCPClient.startTCP(myStress, IPAddress, IPPort) == false)
			return false;
	...
	public void run() 
	{
		...
		while (connected == true) 
		{
			...
			catch (Exception e) 
			{
		        ...
		        if (connected == true)
		        {
			        Message msg = myStress.mHandler.obtainMessage(myStress_remote.SHOW_NOTIFICATION);
			        Bundle bundle = new Bundle();
			        bundle.putString(myStress_remote.TEXT, "It seems that the TCP connection broke. It is recommended to restart the gateway since the server might be temporarily down!");
					msg.setData(bundle);
			        myStress.mHandler.sendMessage(msg);
