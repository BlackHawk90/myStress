 ...
 * @see         myStress_remote
 */
public class myStress_remotevalues extends Activity
{
	 private static final int REFRESH_VALUES = 1;
	 private TextView mTitle;
	 private TextView mTitle2;
	 private TextView mValue1, mValue2;
	 private myStress_remote myStress_remote = null;;
	 ...
	 @Override
	    public void onCreate(Bundle savedInstanceState) 
	    {
	        ...
	        // bind to service
	        if (bindService(new Intent(this, myStress_remote.class), mConnection, 0)==false)
	 ...
	 @Override
	    public boolean onOptionsItemSelected(MenuItem item) 
	    {
	        ...
	        case R.id.remote_exit:
	    		...
	    		builder.setMessage(getString(R.string.Exit_myStress))
	    		       .setTitle(getString(R.string.myStress_Remote_Sensing))
	    		       .setCancelable(false)
	    		       .setPositiveButton(getString(R.string.Yes), new DialogInterface.OnClickListener() 
	    		       {
	    		           public void onClick(DialogInterface dialog, int id) 
	    		           {
	    		       		   SerialPortLogger.debugForced("myStress_remotevalues::Exiting");
	    		    		   if (myStress_remote != null)
	    		    			 stopService(new Intent(act, myStress_remote.class));
	    					   SerialPortLogger.debugForced("myStress_remotevalues::...stopped service");
	    ...
	    private ServiceConnection mConnection = new ServiceConnection() 
	    {
	  	    public void onServiceConnected(ComponentName className, IBinder service) 
	  	    {
	     		...
	     		myStress_remote = ((myStress_remote.LocalBinder)service).getService();
	     		// if start has failed, stop service
	     		if (myStress_remote.failed == true)
	     		{
	    			stopService(new Intent(act, myStress_remote.class));
	     			finish();
	     		}
	     		// if not running, show text
	  	        if (myStress_remote.running == false)
		 ...
		 private final Handler mHandler = new Handler() 
	     {
	        ...
	        {        	
	            ...
	            case REFRESH_VALUES:
	            	if (myStress_remote!=null)
	            	{
						mValue1.setText(String.valueOf(myStress_remote.values_sent));
						mValue2.setText(String.valueOf((float)myStress_remote.bytes_sent/1000));
	            	}
		            break;  
	            default:  
	            	break;
