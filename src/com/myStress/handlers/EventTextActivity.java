...
/** Activity to self-annotate any text being pasted to myStress
...
public class EventTextActivity extends Activity 
{
	...
	public void onCreate (Bundle savedInstanceState) 
	{
    	...
    	Log.v("myStress", "Started ACTION_SEND activity!");
        ...
        if (recentTasks.size() > 1)
        {
        	...
        	if (sender.baseIntent != null)
        	{
        		...
        		Log.v("myStress", "Last recent component: " + caller);
        		...
        		Log.v("myStress", "Last recent program: " + applicationName);
        	}
        }
        else
        	Log.v("myStress", "Just one recent task?");

        
        // if text has been sent to myStress, record it now!
        if (Intent.ACTION_SEND.equals(action)) 
        {
			// send broadcast intent to signal end of selection to mood button handler
			Intent intentmyStress = new Intent("com.myStress.eventtext");
			intentmyStress.putExtra("Text", applicationName + "::" + intent.getStringExtra(Intent.EXTRA_TEXT));			
			sendBroadcast(intentmyStress);			
        }
        finish();
    }
}
