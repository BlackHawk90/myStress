/*
Copyright (C) 2008-2011, Dirk Trossen, myStress@dirk-trossen.de
...
/**
 * Class to read cellular sensors, specifically the CR, CD, CS, CB, CI, CL, CC
 * sensor
 * 
 * @see Handler
 */
public class CellHandler extends PhoneStateListener implements
		com.myStress.handlers.Handler {
	...
	private boolean startedData = false, startedSignal = false,
	...
	public String Share(String sensor) {
		...
		case 'I':
			return "The current cell identifier I am connected to is "
					+ String.valueOf(oldCellID);
		case 'L':
			return "The current cell area identifier I am connected to is "
					+ String.valueOf(oldcellLac);
		case 'R':
			if (roaming == 1)
		...
		case 'S':
			return "The current cell signal strength is "
					+ String.valueOf(oldcellStrength) + " dB";
		case 'B':
			return "The current cell signal strength is "
					+ String.valueOf(oldcellStrength_bar) + " bars";
		case 'D':
			if (olddata_state == 1)
		...
		case 'C':
			return "My current cell network country code is "
					+ String.valueOf(oldNCC);
		}
		return null;
	}
	 ...
	 * com.myStress.platform.SensorRepository}, if the GSM properties have been
	 * found!
	 * 
	 ...
	 * arming the semaphore Then, seeing if the GSM properties are supported
	...
	/**
	 * Method to release all handler resources Here, we release all semaphores
	 * and remove the phone state listener
	 * 
	 ...
	 * 
	 * @param state
	 *            new state that has been detected
	 ...
	 * 
	 * @param location
	 *            Reference to the new {@link android.telephony.CellLocation}
