...
public class AIRS_upload_service extends Service{ // implements MediaHttpUploaderProgressListener {
	...
	/**
	 * Returns current instance of AIRS_upload_service Service to anybody
	 * binding to it
	 * 
	 * @param intent
				...
				while (try_upload == true) {
					try {
						...
						if (right_network == true) {
							Log.v("AIRS",
									"trying to find AIRS recordings directory");
...
//							+ String.valueOf(progress) + "%", notificationIntent);
		...
		case MEDIA_COMPLETE:
			notification = new Notification(R.drawable.notification_icon,
					context.getString(R.string.Sync_upload),
					System.currentTimeMillis());
	...
	private boolean createValueFile(Context context) {
		...
		try {
			...
			while (syncing == true) {
				query = new String(
						"SELECT Timestamp, Symbol, Value from 'airs_values' WHERE Timestamp > "
				...
				for (i = 0; i < number_values; i++) {
					...
					if (set_timestamp == true) {
						...
						// store timestamp
						// force a date format to address Android 4.3 changes
						// that changed zzz to 'BST' and similar
					...
					line_to_write = new String("#"
							+ String.valueOf(currenttime - currentstart) + ";"
							+ symbol + ";" + value + "\n");
	...
	private boolean createNoteFile(Context context) {
		...
		try {
			...
			while (syncing == true) {
				query = new String(
						"SELECT Year, Month, Day, Annotation, created, modified from 'airs_annotations' WHERE created > "
								+ String.valueOf(currenttime)
								+ " AND created < "
	...
	private static Permission insertPermission(Drive service, String fileId) {
		...
		try {
	        ...
	        Log.e("AIRS","Error on setting Permission for GDrive-File: " + e.getMessage());
		}
		return null;
	}
}
